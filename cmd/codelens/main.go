...
const cacheFileName = ".codelens-cache/graph.json"

func main() {
	// ── Flags ──────────────────────────────────────────────────────────────────
	port := flag.Int("port", 5555, "Local server port")
	noOpen := flag.Bool("no-open", false, "Don't auto-open browser after starting")
	useCache := flag.Bool("cache", true, "Cache analysis to .codelens-cache/graph.json")
	...
	llmModel := flag.String("llm-model", "", "Model name (default: nvidia/nemotron-3-super-120b-a12b for nvidia-nim)")
	...
	flag.Usage = func() {
		fmt.Fprintf(os.Stderr, "Usage: codelens [path] [flags]\n\n")
...
// 1. Explicit CLI flag
// 2. Process environment variables (MODEL_API_KEY, CODELENS_LLM_KEY, NVIDIA_API_KEY, OPENAI_API_KEY)
...
func findAPIKey(flagKey, repoPath string) string {
	...
	envVars := []string{"MODEL_API_KEY", "CODELENS_LLM_KEY", "CODEATLAS_LLM_KEY", "NVIDIA_API_KEY", "OPENAI_API_KEY"}
